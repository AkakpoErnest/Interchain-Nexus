...
async function checkBalance(network, address) {
  try {
    // Fix address checksum if needed
    let validAddress;
    try {
      validAddress = ethers.getAddress(address);
    } catch (checksumError) {
      // Try to fix checksum by converting to lowercase and back
...
async function main() {
  ...
  results.forEach(result => {
    ...
    if (result.address && result.address !== walletAddress) {
      console.log(`   Corrected Address: ${result.address}`);
    }
  });
